...
const idPlaza = new URLSearchParams(window.location.search).get('id');
...
const plazaFicha = document.getElementById('plaza-ficha');
const tituloPlaza = document.getElementById('titulo-plaza');
const btnEditarPlaza = document.getElementById('btn-editar-plaza');
const formEditarPlaza = document.getElementById('form-editar-plaza');
const btnCancelarEditarPlaza = document.getElementById('btn-cancelar-editar-plaza');
const editarPlazaNombre = document.getElementById('editar-plaza-nombre');

async function cargarPlaza() {
  const res = await fetch(`/api/empresas/${encodeURIComponent(idPlaza)}`);
  if (!res.ok) {
    plazaFicha.innerHTML = '<p>Plaza no encontrada.</p>';
    return;
  }
  plazaActual = await res.json();
  tituloPlaza.textContent = plazaActual.empresa;
  document.title = `${plazaActual.empresa} · CRM-ON`;
  plazaFicha.innerHTML = campoFicha('Nombre', plazaActual.empresa);
}

btnEditarPlaza.addEventListener('click', () => {
  editarPlazaNombre.value = plazaActual.empresa || '';
  formEditarPlaza.hidden = false;
  btnEditarPlaza.hidden = true;
});

btnCancelarEditarPlaza.addEventListener('click', () => {
  formEditarPlaza.hidden = true;
  btnEditarPlaza.hidden = false;
});

formEditarPlaza.addEventListener('submit', async (e) => {
  e.preventDefault();
  const res = await fetch(`/api/empresas/${encodeURIComponent(idPlaza)}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ empresa: editarPlazaNombre.value.trim() }),
  });
  if (!res.ok) {
...
const listaDestinosPlaza = document.getElementById('lista-destinos-plaza');
...
opcionesDestinoAgregar.addEventListener('click', async (e) => {
  ...
  if (!id) return;
  const res = await fetch(`/api/empresas/${encodeURIComponent(idPlaza)}/destinos`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
listaDestinosPlaza.addEventListener('click', async (e) => {
  if (!e.target.classList.contains('btn-quitar-destino')) return;
  const id = e.target.dataset.id;
  await fetch(`/api/empresas/${encodeURIComponent(idPlaza)}/destinos/${encodeURIComponent(id)}`, { method: 'DELETE' });
  await cargarAsociados();
});
...
async function cargarAsociados() {
  const res = await fetch(`/api/empresas/${encodeURIComponent(idPlaza)}/asociados`);
  asociadosActual = await res.json();

  contadorDestinos.textContent = `(${asociadosActual.destinos.length})`;
  listaDestinosPlaza.innerHTML = asociadosActual.destinos.length
...
promesaAuth.then(async (sesion) => {
  if (!sesion) return;
  if (!idPlaza) {
    plazaFicha.innerHTML = '<p>Falta el id de la plaza.</p>';
    return;
  }
  permisosCatalogos = sesion.permisos.catalogos;
  formEditarPlaza.hidden = true;
  btnEditarPlaza.hidden = !permisosCatalogos.editar;
  ...
  suscribirTiempoReal(['empresas'], cargarPlaza);
  suscribirTiempoReal(['destinos', 'destino_empresas'], () => { cargarDestinosDisponibles(); cargarAsociados(); });
